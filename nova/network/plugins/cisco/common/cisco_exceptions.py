...
#    Rohit Agarwalla, Cisco Systems, Inc.
...
class NexusConnectFailed(exception.NovaException):
    """Failed to connect to Nexus switch."""
    message = _("Unable to connect to Nexus %(nexus_host)s. Reason: %(exc)s.")


class NexusConfigFailed(exception.NovaException):
    """Failed to configure Nexus switch."""
    message = _("Failed to configure Nexus: %(config)s. Reason: %(exc)s.")


class CiscoNEXUSUnsupportedEtype(exception.NovaException):
    message = _("Unsupported Interface Type %(etype)s")
